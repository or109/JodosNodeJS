...
Bayesian.prototype = {
	...
	getProbsSync: function(document) {
		...
		var probs = this.getCatProbs(cats, words, counts); // The probabilities that the given document belongs to each of the categories, i.e.: { 'cat1': 0.1875, 'cat2': 0.0625 }
			...
			console.log("This is probs -  " );
			sys.puts(sys.inspect(probs, false, null));
	...
	getCatProbs: function(cats, words, counts) {
		...
		_(cats).each(function(catCount, cat) {
			...
			var x = catProb.toFixed(15);
			var y = (docProb.toFixed(20) == 0) ? (parseFloat("0.00000000000001") + docProb).toFixed(15) : docProb.toFixed(15);
			console.log(x + "*" + y);
			var ofir = x * y;
			console.log("cat " + cat + "   catProbs " + catProb + " docProb - " + docProb + " and * - "  + ofir);
			
			probs[cat] = catProb * docProb;
		}, this);
		console.log("This is inner probs -  " );
		sys.puts(sys.inspect(probs, false, null));
		return probs;
	},
	...
	incDocCounts: function(samples, callback) {
			...
			console.log("This is catIncs -  " );
			sys.puts(sys.inspect(catIncs, false, null));
			console.log("This is wordIncs -  " );
			sys.puts(sys.inspect(wordIncs, false, null));
